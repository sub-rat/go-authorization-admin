...
import {
  ...
  useRoleServicePostApiV1Roles,
  useRoleServicePutApiV1Roles,
} from "@/services/generated/queries"

import { IRoleModal } from "../../types"
import {
  models_MenuQueryResult,
  models_RoleMenu,
} from "../../../../../../services/generated/requests"
import { Switch } from "../../../../../common/switch"
const RoleModal = ({ open, onClose, editData }: IRoleModal) => {
  const methods = useForm({
    defaultValues: {
      ...editData,
      rolesIds: [...new Set(editData?.role_menus?.map((x) => x.menu_id!))].map(
        (x) => ({ value: x }),
      ) as [],
    },
  })
  ...
  const rolesList = [
    ...new Set(
      methods.watch("rolesIds")?.map((x: { value: string }) => x?.value),
    ),
  ]
                          ...
                          {e?.actions?.map((g) => {
                            return (
                              <Checkbox
                                ...
                                label={g.name}
                                // value={g.id}
                                defaultChecked={editData?.role_menus
                                  ?.map((x) => x.action_id)
                                  .includes(g.id!)}
                                // defaultChecked={g.id}
