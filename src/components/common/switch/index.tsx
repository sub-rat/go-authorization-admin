            ...
            <div className="flex items-start gap-2">
                ...
                {(label || subLabel || badge || description) && (
                    <div className="flex flex-col gap-1 justify-center">
                        <span>
                            <label className="text-lg font-medium hover:cursor-pointer" htmlFor={props.id || id}>
                                {label}
                                <span className="text-sm inline-block ml-2 text-secondary leading-normal font-normal">{subLabel}</span>
                            </label>
                            <span className="ml-2">{badge}</span>
                        </span>
                        <span className="text-sm leading-normal text-secondary">{description}</span>
                    </div>
                )}
            </div>
        );
