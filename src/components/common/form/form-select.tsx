...
interface IFormSelectProps<T extends Record<string, unknown>>
  extends SelectProps {
  control?: Control<T>
  name?: string
  showAll?: boolean
...
export const FormSelect = <T extends Record<string, unknown>>(
  ...
  return (
    <Controller
      ...
      name={name ?? ""}
      render={({ field }) => (
        <FormControl field={field} errors={errors} {...props} />
      )}
    />
  )
}

interface FormControlProps<T extends Record<string, unknown>>
  extends IFormSelectProps<T> {
